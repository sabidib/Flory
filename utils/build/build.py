...
def getHeader():
    ...
    with open("header.txt") as f:
        runningString = f.read();
    return runningString;


def filterLines(lines):
    #Match jslint comments out
    newLines = ""
    splitLines = lines.split("\n");
    for line in splitLines:
        if(re.match("^\/\*global.*\*\/", line)):
            line = "";
        if(re.match("^\/\*jslint.*\*\/", line)):
            line = "";
        newLines += line+"\n";
    newLines = newLines.replace("'use strict';","");
    newLines = newLines.replace('"use strict";',"");
    newLines = "'use strict';" + "\n"*3 + newLines;
    return newLines;

def compileDemos(minify,source_order):
    print "";
    ...
    for demo in data:
        ...
        for script in demo['scripts']:
            ...
            with open(script_location) as f:
                a = f.read();
            runningString += a + "\n";
        runningString = filterLines(runningString);
        runningString = getHeader() + runningString;
        final_build_location_file.write(runningString);
...
def compileBaseSource(minify,source_order):
    ...
    print "";
    print "Loading source file order from",source_order;

    with open(source_order,'r') as f:
        data = json.load(f);
...
def parseArgs(argv):
    ...
    parser.add_option("-s", "--no-external-libaries",action="store_true",default=False,dest="noextern",help="do not include external libaries.")
...
def main(argv):
    ...
    noextern = options.noextern;
    source_order = "source_order.cfg";
    if(noextern):
        source_order = "flory_standalone.cfg";
    ...
    if(base):
        ...
        compileBaseSource(minify = minify,source_order=source_order);
    ...
    if(demos):
        ...
        compileDemos(minify = minify,source_order=source_order);
        print divider_string
